...
from .validation import default_sm_scale, validate_decode_attn_inputs


def pytorch_decode_attn_contig(q, k, v, seq_lens, sm_scale=None):
    ...
    k_f = k.float()
    v_f = v.float()
    # Keep the PyTorch oracle simple, so ragged lengths drive Python loops.
    seq_lens_host = seq_lens.to(dtype=torch.int64).detach().cpu()
    out = torch.empty((B, Hq, D), device=q.device, dtype=torch.float32)

    for b in range(B):
        seq_len = int(seq_lens_host[b])
        for hq in range(Hq):
            if seq_len == 0:
                out[b, hq].zero_()
                continue
